...
export function DeviceTab() {
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-border bg-card shadow-card">
      <div
        className="flex flex-wrap gap-1 border-b border-border bg-muted/50 p-2"
        role="tablist"
        aria-label="TechForges device information"
      >
        {tabs.map((t) => (
          <button
            key={t}
        ...
        <div>
          ...
          {tab === "Sync with LabelTruth" && (
            <div
              ...
              role="tabpanel"
              tabIndex={0}
              aria-label="Sync with LabelTruth"
              className="space-y-4 text-sm leading-relaxed text-muted-foreground"
            >
              <p className="text-foreground">
                <strong className="font-semibold">One scan, two signals.</strong> Every pack you
                scan is pushed to the tracker so intake is logged with its P:C ratio and trust score
                attached.
              </p>
