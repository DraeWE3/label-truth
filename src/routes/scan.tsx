...
function Scan() {
  ...
  async function handleFiles(files: FileList | null, input?: HTMLInputElement) {
    ...
    try {
      ...
      setResult(res);
      if (res.isFoodLabel) saveScan({ image: dataUrls[0]!, result: res });
    } catch (e) {
        ...
        >
          <div className="mx-auto max-w-3xl">
            ...
            <p className="mt-4 max-w-xl text-sm leading-relaxed text-primary-foreground/75">
              For the most accurate reading, add up to three photos of the same product: the front
              of pack, the ingredient list and the nutrition panel. More readable text means a
              higher confidence band.
            </p>
            ...
            <p id="scan-help" className="mt-3 text-xs text-primary-foreground/65">
              Photos are sent to our AI provider to create your reading. The latest scans are also
              saved on this device only, and you can clear them from History.
            </p>
            <input
              ref={cameraRef}
              type="file"
