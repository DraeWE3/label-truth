 ...
 *  Set AI_PROVIDER=workers-ai|lovable|openai|gemini to force one.
 *
...
export function resolveAiProvider(): AiProvider {
  ...
  // Optional override (workers-ai | lovable | openai | gemini) so a key that is
  // present for one provider does not silently win over the one you want.
  const forced = readEnv("AI_PROVIDER")?.trim().toLowerCase();
  const allow = (name: string) => !forced || forced === name;
  if (allow("workers-ai") && cfAccountId && cfApiToken) {
    return {
      name: "workers-ai",
  ...
  if (allow("gemini") && gemini) {
    const model = sanitizeGeminiModel(readEnv("AI_MODEL") ?? "gemini-3.6-flash");

    return {
      name: "gemini",
      transport: "gemini-content",
      url: `${readEnv("GEMINI_API_BASE") ?? "https://generativelanguage.googleapis.com"}/v1beta/models/${model}:generateContent`,
      headers: {
        "Content-Type": "application/json",
